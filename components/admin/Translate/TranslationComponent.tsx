...
const TranslationComponent = ({
  ...
  const errClass = (isError: boolean) =>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 lg:gap-6">
        <div className="flex flex-col gap-2 border shadow shadow-black bg-white p-3 lg:p-4">
          ...
          <input
            ...
            className={`h-9 lg:h-10 w-full pl-2 outline-none text-xs lg:text-sm ${errClass(
              !!validationErrors.englishTitle,
            )}`}
          />
        </div>
        <div className="flex flex-col gap-2 border shadow shadow-black bg-white p-3 lg:p-4">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4 lg:gap-6">
        <div className="flex flex-col gap-2 border shadow shadow-black bg-white p-3 lg:p-4">
          ...
          <textarea
            ...
            rows={4}
            className={`min-h-15 lg:min-h-20 w-full resize-y p-2 outline-none text-xs lg:text-sm ${errClass(
              !!validationErrors.englishDescription,
            )}`}
          />
        </div>
        <div className="flex flex-col gap-2 border shadow shadow-black bg-white p-3 lg:p-4">
          ...
          <textarea
            ...
            rows={4}
            className={`min-h-15 lg:min-h-20 w-full resize-y p-2 outline-none text-xs lg:text-sm ${errClass(
              !!validationErrors.nepaliDescription,
            )}`}
          />
        </div>
      </div>
        ...
        {englishBlocks.length === 0 ? (
          ...
          <div className="flex flex-col gap-2 lg:gap-3">
            {englishBlocks.map((block, index) => {
              ...
              const blockError = validationErrors.blocks[index]; // <-- add this

              return (
                <div
                  key={block.id}
                  className="border shadow shadow-black bg-white p-3 lg:p-4"
                >
                  ...
                  {block.type === "heading" && (
                    <>
                      <div className="flex gap-1 flex-wrap mb-2">
                        {[1, 2, 3, 4, 5].map((l) => (
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              updateEnglishBlock(index, { level: l })
                            }
                      ...
                      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                        <textarea
                          className={`min-h-16 w-full resize-y p-2 outline-none text-xs lg:text-sm ${errClass(
                            !!blockError?.english,
                          )}`}
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            updateNepaliValue(index, e.target.value)
                          }
                        />
                      </div>
                    </>
                  ...
                  {block.type === "paragraph" && (
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                      <textarea
                        ...
                        value={block.content ?? ""}
                        onChange={(e) =>
                          updateEnglishBlock(index, { content: e.target.value })
                        }
                      />
                      <textarea
                        className={`min-h-16 w-full resize-y p-2 outline-none text-xs lg:text-sm ${errClass(
                  ...
                  {block.type === "callout" && (
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                      <div className="border bg-[#EBECD8]/50 p-3">
                        <input
                          className={`w-full font-bold outline-none bg-transparent mb-2 text-xs lg:text-sm p-1 ${errClass(
                            !!blockError?.english,
                          )}`}
                        ...
                        />
                        <textarea
                          className={`w-full outline-none bg-transparent resize-none text-xs lg:text-sm p-1 ${errClass(
                            !!blockError?.english,
                          )}`}
                      ...
                      <div className="border bg-[#EBECD8]/50 p-3">
                        <input
                          className={`mb-2 w-full bg-transparent text-xs lg:text-sm font-bold outline-none p-1 ${errClass(
                            !!blockError?.nepali,
                          )}`}
                          value={(nepaliValue as any)?.title ?? ""}
                          placeholder="Callout title (Nepali)"
                          onChange={(e) =>
                            updateNepaliCallout(index, {
                              title: e.target.value,
                            })
                          }
                        />
                  ...
                  {block.type === "list" && (
                    <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                      <div className="flex flex-col gap-2 border bg-[#EBECD8]/50 p-2">
                        {(Array.isArray(block.content)
                          ...
                          <div key={i} className="flex gap-1 items-center">
                            ...
                            <input
                              className={`flex-1 outline-none p-1 bg-white text-xs lg:text-sm ${errClass(
                                !!blockError?.english ||
                                  !!blockError?.englishItems?.includes(i),
                              )}`}
                              value={item}
                              placeholder={`English item ${i + 1}`}
                              onChange={(e) =>
                                updateEnglishListItem(index, i, e.target.value)
                              }
                            />
                            <button
                              type="button"
                      ...
                      <div className="flex flex-col gap-2 border bg-[#EBECD8]/50 p-2">
                        <textarea
                          className={`min-h-24 w-full resize-y bg-white p-2 text-xs lg:text-sm outline-none ${errClass(
                            !!blockError?.nepali ||
                              !!blockError?.nepaliItems?.length,
                          )}`}
                          placeholder="One item per line. Press Enter to add another."
                          value={getNepaliListText(nepaliValue)}
                          onChange={(e) =>
                            handleNepaliListTextChange(index, e.target.value)
                          }
                        />
                        {(blockError?.english || blockError?.nepali) && (
                          <p className="text-[11px] text-red-600">
                            Ensure both English and Nepali lists have the same number of non-empty items.
                          </p>
                        )}
