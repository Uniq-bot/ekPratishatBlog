          ...
          <div className="text-center">
            <p
              className={`mx-auto mb-3 inline-flex rounded-full border border-[#eadcb4] bg-[#fffaf0] px-4 py-1 font-semibold uppercase text-[#8a6b12] ${
                isEnglish ? "text-xs tracking-[0.22em]" : "text-sm tracking-normal"
              }`}
            >
              {isEnglish ? "Your Guide to Real Estate" : "रियल-स्टेटको लागि तपाईंको मार्गदर्शक"}
            </p>
            <div className="flex flex-wrap items-center justify-center gap-2 leading-none sm:gap-4">
              <h1
                className={`font-black ${
                  isEnglish ? "text-[1.75rem] sm:text-[2.5rem] md:text-[4rem] lg:text-[5rem] xl:text-[6rem]" : "text-[2.2rem] sm:text-[3rem] md:text-[4.5rem] lg:text-[5.6rem] xl:text-[6.5rem]"
                }`}
              >
                {isEnglish ? "REAL-ESTATE" : "घरजग्गा"}
              </h1>
              <span className={`flex flex-col font-black leading-none ${isEnglish ? "text-[1.75rem] sm:text-[2rem] lg:text-[3rem]" : "text-[1.95rem] sm:text-[2.4rem] lg:text-[3.4rem]"}`}>
                <span className="text-transparent [-webkit-text-stroke:2px_#f1c810]">{isEnglish ? "MADE" : "सम्बन्धी"}</span>
                <span className="bg-clip-text text-transparent bg-[#c7c7c6] [-webkit-text-stroke:1px_#7b7b7b]">{isEnglish ? "CLEAR" : "स्पष्ट जानकारी"}</span>
              </span>
            </div>
          </div>
          ...
          <div className="border border-[#eadcb4] bg-white/90 p-5 shadow-[0_18px_45px_rgba(15,23,42,0.06)] sm:p-6 lg:p-7">
            ...
            <div className="mt-3 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
              <div className="max-w-3xl space-y-2">
                <h2 className={`font-black leading-tight text-black ${isEnglish ? "text-2xl sm:text-3xl lg:text-4xl" : "text-[1.45rem] sm:text-[1.8rem] lg:text-[2.2rem]"}`}>
                  {isEnglish
                ...
                <p className={`max-w-2xl leading-6 text-black/65 ${isEnglish ? "text-sm sm:text-base" : "text-base sm:text-lg"}`}>
                  {isEnglish
